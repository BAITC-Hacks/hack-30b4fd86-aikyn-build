...
Показать жюри: запуск `local_eval.py`, чистый результат и соблюдение лимитов, затем `submission.csv` и повторную генерацию. `verify_pass.py` проверяет повторяемость файла и выводит JSON-отчёт; для сохранения перенаправьте вывод в `validation.json`.
...
Важно: поле «Кампаний» в отчёте local_eval включает пилоты. Например, 24 = 20 пилотов + 5 финальных кампании. Строки «Осталось бюджета/охвата» показывают остаток после пилотов; итоговые расходы смотрите в общей таблице результата.

## Первый локальный результат

На seed=42: 5 финальных кампании, 20 пилотов, чистый прирост 1222447 у.е., расходы 31488, контакты 7872. На seed 0–9: 10 из 10 результатов положительные; минимум 1247717, медиана 1602548, максимум 1730401 у.е. Округлено до целых.
